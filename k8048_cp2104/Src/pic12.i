...
{"PIC12F508", (12508), 512, 0, (41227), 0x0204, 0x03FF, 1},
{"PIC12F509", (12509), 1024, 0, (41227), 0x0404, 0x07FF, 1},

{"PIC16F505", (16505), 1024, 0, (41226), 0x0404, 0x07FF, 1},

{"PIC12F510", (12510), 1024, 0, (41257), 0x0404, 0x07FF, 1},

{"PIC16F506", (16506), 1024, 0, (41258), 0x0404, 0x07FF, 1},

{"PIC10F200", (10200), 256, 0, (41228), 0x0104, 0x01FF, 1},
{"PIC10F202", (10202), 512, 0, (41228), 0x0204, 0x03FF, 1},
{"PIC10F204", (10204), 256, 0, (41228), 0x0104, 0x01FF, 1},
{"PIC10F206", (10206), 512, 0, (41228), 0x0204, 0x03FF, 1},

{"PIC16F54", (1654), 512, 0, (41207), 0, 0x03FF, 1},

{"PIC16F57", (1657), 2048, 0, (41208), 0, 0x0FFF, 4},

{"PIC16F59", (1659), 2048, 0, (41243), 0, 0x0FFF, 4},

{"PIC10F220", (10220), 256, 0, (41266), 0x0104, 0x01FF, 1},
{"PIC10F222", (10222), 512, 0, (41266), 0x0204, 0x03FF, 1},

{"PIC12F519", (12519), 1024, +64, (41316), 0x0444, 0x07FF, 1},

{"PIC16F570", (16570), 2048, +64, (41670), 0x0844, 0x0FFF, 4},

{"PIC16F527", (16527), 1024, +64, (41640), 0x0444, 0x07FF, 1},

{"PIC16F526", (16526), 1024, +64, (41317), 0x0444, 0x07FF, 1},
...
uint32_t pic12_index = ((sizeof(pic12_map) / sizeof(struct pic12_dsmap)) - 1);

void
pic12_selector(void)
{
 uint32_t i;
 char *dnames[((sizeof(pic12_map) / sizeof(struct pic12_dsmap)) - 1)];

 for (i = 0; i < ((sizeof(pic12_map) / sizeof(struct pic12_dsmap)) - 1); ++i) {
  dnames[i] = pic12_map[i].devicename;
 }
 qsort(dnames, ((sizeof(pic12_map) / sizeof(struct pic12_dsmap)) - 1), sizeof(char *), pic_cmp);
 for (i = 0; i < ((sizeof(pic12_map) / sizeof(struct pic12_dsmap)) - 1); ++i) {
  if ((i % (5)) == ((5) - 1))
   printf("%s\n", dnames[i]);
  else
 ...
 if (i % (5))
  printf("\n");
 printf("Total: %d\n", (uint32_t)((sizeof(pic12_map) / sizeof(struct pic12_dsmap)) - 1));
}
# 165 "pic12.c"
void
...
pic12_end_programming(struct k8048 *k)
{
 ...
 if (pic12_map[pic12_index].datasheet == (41670))
  tdis = (300);
...
pic12_read_config_memory(struct k8048 *k, int flag)
{
 ...
 pic12_index = ((sizeof(pic12_map) / sizeof(struct pic12_dsmap)) - 1);
...
pic12_program(struct k8048 *k, char *filename, int blank)
{
 ...
 if (blank)
  pic12_bulk_erase(k, (0xFFFF), (0xFFFF));
 ...
 for (uint32_t i = 0; i < k->count; i++) {
  ...
  if (new_region != current_region) {
   if (current_region == (1))
    pic12_programregion(k, (0xFFFF), (1), (0xFFFF));
   pic12_program_verify(k);
   if (pic12_initregion(k, new_region, &PC_address) == (0))
    continue;
  ...
  while (hex_address > PC_address) {
   if (current_region == (1))
    pic12_programregion(k, PC_address, (1), (0xFFFF));
   PC_address++;
   pic12_increment_address(k);
  }
...
pic12_dumpdeviceid(struct k8048 *k)
{
 ...
 for (uint32_t i = 0; i < 4; i++) {
  printf("[%04X] [USERID%d]  %04X %c\n",
   ...
   pic12_conf.userid[i], ((((0xFF & pic12_conf.userid[i]) >= ' ' && (0xFF & pic12_conf.userid[i]) < 127) ? (0xFF & pic12_conf.userid[i]) : ('.'))));
 }
...
pic12_dumphexcode(struct k8048 *k, uint32_t address, uint32_t size, uint32_t *data)
{
 ...
 for (i = 0; i < size; address += 8, i += 8) {
  if (pic_mtcode((0x0FFF), 8, &data[i]))
   continue;
  printf("[%04X] ", address);
  for (j = 0; j < 8; ++j)
   printf("%03X ", data[i + j] & (0x0FFF));
  for (j = 0; j < 8; ++j)
...
pic12_dumpdevice(struct k8048 *k)
{
 ...
 pic_dumpword16((0x0FFF), pic12_conf.config);
}
