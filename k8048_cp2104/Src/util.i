...
mystrcasestr(const char *haystack, const char *needle)
{
 ...
 for (i = 0; i <= hlen; i++) {
  for (j = 0; j < nlen; j++) {
   if ((__extension__ ({ int __res; if (sizeof ((int)haystack[i + j]) > 1) { if (__builtin_constant_p ((int)haystack[i + j])) { int __c = ((int)haystack[i + j]); __res = __c < -128 || __c > 255 ? __c : (*__ctype_tolower_loc ())[__c]; } else __res = tolower ((int)haystack[i + j]); } else __res = (*__ctype_tolower_loc ())[(int) ((int)haystack[i + j])]; __res; })) != (__extension__ ({ int __res; if (sizeof ((int)needle[j]) > 1) { if (__builtin_constant_p ((int)needle[j])) { int __c = ((int)needle[j]); __res = __c < -128 || __c > 255 ? __c : (*__ctype_tolower_loc ())[__c]; } else __res = tolower ((int)needle[j]); } else __res = (*__ctype_tolower_loc ())[(int) ((int)needle[j])]; __res; })))
    break;
  }
  if (j == nlen) {
   return &haystack[i];
