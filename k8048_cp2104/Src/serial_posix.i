...
serial_open(const char *device)
{
 ...
 serial_port = open(device, 02 | 0400 | 04000);
 if (serial_port < 0) {
  serial_port = -1;
  return -1;
 }

 fcntl(serial_port, 4, 04000);
 ...
 options.c_cflag = 0000001 | 0000004 | 0000060 | 0000200 | 0004000;
 ...
 tcsetattr(serial_port, 0, &options);
...
get_cts(void)
{
 ...
 unsigned gpio;
 ioctl(serial_port, 0x8000, &gpio);
 return (gpio & 0x00000008 ) ? ((1)) : ((0));
