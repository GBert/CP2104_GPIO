...
{
 if (k->pic->arch == (0x00003FFF)) {
  pic_read_config(k, (0));

  if (k->pic->bulk_erase)
   k->pic->bulk_erase(k, (0xFFFF), bandgap);
  else
   printf("%s: information: unimplemented\n", __func__);
 } else
...
{
 if (k->pic->arch == (0x00000FFF) || k->pic->arch == (0x00003FFF)) {
  pic_read_config(k, (0));

  if (k->pic->bulk_erase)
   k->pic->bulk_erase(k, osccal, (0xFFFF));
  else
   printf("%s: information: unimplemented\n", __func__);
 } else
...
pic_blank(struct k8048 *k)
{
 if (k->pic->bulk_erase) {
  ...
  k->pic->bulk_erase(k, (0xFFFF), (0xFFFF));
 } else
  printf("%s: information: unimplemented\n", __func__);
}
...
pic_dumpprogram(struct k8048 *k, uint32_t size)
{
 ...
 pic_dump_program(k, faddr, size, (1));
}
...
pic_dumpexec(struct k8048 *k)
{
 ...
 if (esize == (4294967295U) || esize == 0) {
  printf("%s: fatal error: EXECUTIVE size invalid\n", __func__);
  io_exit(k, 70);
 }

 pic_dump_program(k, eaddr, esize, (1));
}
# 553 "pic.c"
void
pic_dump_program(struct k8048 *k, uint32_t addr, uint32_t size, int mode)
{
 ...
 if (pic_read_program_memory_block(k, data, addr, size) == (4294967295U)) {
...
pic_dump_data(struct k8048 *k, uint32_t addr, uint32_t size, int mode)
{
 ...
 if (pic_read_data_memory_block(k, data, addr, size) == (4294967295U)) {
