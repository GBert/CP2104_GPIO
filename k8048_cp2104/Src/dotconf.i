...
{
 char dir[(1024)], *filename, *homedir, *username;

 filename = getenv("K8048");
 if (filename != ((void *)0) && access(filename, 4) == 0) {
  snprintf(k->dotfile, (1024), "%s", filename);
  return;
 }
 ...
 snprintf(k->dotfile, (1024), "%s/%s", dir, ".k8048");
 if (access(k->dotfile, 4) == 0) {
  return;
 }

 homedir = getenv("HOME");
 if (homedir != ((void *)0)) {
  snprintf(k->dotfile, (1024), "%s/%s", homedir, ".k8048");
  if (access(k->dotfile, 4) == 0) {
   return;
  }
 }
 ...
 if (username != ((void *)0)) {
  snprintf(k->dotfile, (1024), "/home/%s/%s", username, ".k8048");
  if (access(k->dotfile, 4) == 0) {
   return;
  }
 }
...
getconf(struct k8048 *k, char *e)
{
 ...
 k->bitrules = ((0x0010) | (0x0001) | (0x0002) | (0x0004) | (0x0008));
 k->sleep_low = 1;
 k->sleep_high = 1;
 k->fwsleep = 30;
 ...
 if (k->dotfile[0] && (f1 = fopen(k->dotfile, "rb")) != ((void *)0)) {

  while (fgets(line, (1024), f1) != ((void *)0)) {
   ...
   if (mystrcasestr(line, "DEVICE=") == line) {
    ...
    strncpy(k->device, &line[7], (1024));
   }
   else if (mystrcasestr(line, "SLEEP=") == line) {



    k->sleep_low = strtoul(&line[6], ((void *)0), 0);
    ...
    k->sleep_low = strtoul(&line[10], ((void *)0), 0);
   }
   else if (mystrcasestr(line, "SLEEP_HIGH=") == line) {



    k->sleep_high = strtoul(&line[11], ((void *)0), 0);
   }
   else if (mystrcasestr(line, "BITRULES=") == line) {



    k->bitrules = strtoul(&line[9], ((void *)0), 0);
   }
   else if (mystrcasestr(line, "BUSY=") == line) {



    k->busy = strtoul(&line[5], ((void *)0), 0);
   }
# 192 "dotconf.c"
    ...
    k->fwsleep = strtoul(&line[8], ((void *)0), 0);
   }
   else if (mystrcasestr(line, "DEBUG=") == line) {



    k->debug = strtoul(&line[6], ((void *)0), 0);
   }
  }
  fclose(f1);
 } else {
  snprintf(k->dotfile, (1024), "Using defaults (override in %s)", ".k8048");
 }
}
